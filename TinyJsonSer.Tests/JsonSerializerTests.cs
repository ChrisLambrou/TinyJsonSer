...
namespace TinyJsonSer.Tests
{
    ...
    public class JsonSerializerTests
    {
        ...
        /// </summary>
        private Func<object, string> CreateTinyJsonSerializer(bool indented) => new JsonSerializer(indented).Serialize;
        ...
        private Func<object, string> CreateReferenceSerializer(bool indented)
            => target => Newtonsoft.Json.JsonConvert.SerializeObject(target, new Newtonsoft.Json.JsonSerializerSettings
            {
        ...
        private static IEnumerable<TestCaseData> TestCases()
        {
            ...
            yield return new TestCaseData(true, new DateTime(1975, 4, 23, 8, 14, 23, DateTimeKind.Utc)).SetName("DateTime (UTC)");
            yield return new TestCaseData(true, new DateTime(1975, 4, 23, 8, 14, 23, DateTimeKind.Local)).SetName("DateTime (Local)");
            yield return new TestCaseData(true, new DateTime(1975, 4, 23, 8, 14, 23, DateTimeKind.Unspecified)).SetName("DateTime (Unspecified)");
            ...
            yield return new TestCaseData(true, new DateTimeOffset(new DateTime(1975, 4, 23, 8, 14, 23, DateTimeKind.Utc))).SetName("DateTimeOffset (UTC)");
            yield return new TestCaseData(true, new DateTimeOffset(new DateTime(1975, 4, 23, 8, 14, 23, DateTimeKind.Local))).SetName("DateTimeOffset (Local)");
            yield return new TestCaseData(true, new DateTimeOffset(new DateTime(1975, 4, 23, 8, 14, 23, DateTimeKind.Unspecified))).SetName("DateTimeOffset (Unspecified)");
            ...
            yield return new TestCaseData(true, FlagsEnum.Alpha | FlagsEnum.Bravo).SetName("enum (flags, combined value)");
            ...
            for (int i = 0; i < 32; i++)
            {
                yield return new TestCaseData(true, new string(new[] { (char)i })).SetName($"string (control code {i:x2})");
            }
            ...
            yield return new TestCaseData(true, new object[] { "abc", new object[] { true, false }, 123 }).SetName("array (nested elements)");
            ...
            yield return new TestCaseData(true, new { abc = 123, def = new { ghi = 456 } }).SetName("object (nested objects)");
            yield return new TestCaseData(true, new { ABC = 123 }).SetName("object (camel-case, all uppercase)");
            yield return new TestCaseData(true, new { FooBar = 123 }).SetName("object (camel-case, two words, both with capital letter)");
            yield return new TestCaseData(true, new { SQLServer = 123 }).SetName("object (camel-case, two words, first is all uppercase)");

            yield return new TestCaseData(true, new Dictionary<string, object> { ["abc"] = 123, ["def"] = "ghi" }).SetName("dictionary (of string and object)");
            yield return new TestCaseData(true, new Dictionary<string, object>()).SetName("dictionary (of string and object - empty)");
            yield return new TestCaseData(true, new Dictionary<object, object> { ["abc"] = 123, ["def"] = "ghi" }).SetName("dictionary (of object and object)");
            yield return new TestCaseData(true, new Dictionary<object, object> { [10] = 123, [20] = "ghi" }).SetName("dictionary (of object and object, with non-string keys)");
        ...
        public void CheckCompact(bool expectExactStringMatch, object target) => Check(expectExactStringMatch, target, false);

        [Test]
        [TestCaseSource(nameof(TestCases))]
        public void CheckIndented(bool expectExactStringMatch, object target) => Check(expectExactStringMatch, target, true);
